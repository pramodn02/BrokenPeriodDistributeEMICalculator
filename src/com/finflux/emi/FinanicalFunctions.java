...
public class FinanicalFunctions {
    ...
    public static double pmtDaily(final double interestRatePerDay, final LocalDate interestStartDate, final List<LocalDate> repaymentDates,
            final double principal) {
        List<Double> periodInterestRates = new ArrayList<>(repaymentDates.size());
        LocalDate startDate = interestStartDate;
        for (LocalDate dueDate : repaymentDates) {
//            System.out.println(startDate +"due date "+dueDate);
            long days = ChronoUnit.DAYS.between(startDate, dueDate);
            periodInterestRates.add(1 + (interestRatePerDay * days));
            startDate = dueDate;
        }
        double numerator = periodInterestRates.remove(0);
        Collections.reverse(periodInterestRates);

        double denominator = 1;
        double multiplicationVal = 1;
        for (double periodRate : periodInterestRates) {
            multiplicationVal = multiplicationVal * periodRate;
            denominator = denominator + multiplicationVal;
        }
        numerator = numerator * multiplicationVal;
